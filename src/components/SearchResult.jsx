...
export default function SearchResult({searchQuery,setResultBox}) {
    ...
    useEffect(()=> {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${BASE_URL}/search/multi?api_key=${API_KEY}&query=${searchQuery}`)
  ...
  return (
    <div className='position-absolute top-25 mt-2 text-white py-2 bg-dark searchResult' style={{height:'350px' , overflowY: 'scroll'}}>
        {error && <p className='mt-4 fs-5'>{error.message}</p>}
        {filteredResults.length > 0 ? (
            <>
            {filteredResults.map((result)=> (
              <Link to={result.title ? `/movie/${result.id}` :`/person/${result.id}`}
                onClick={()=> setResultBox(false)}  key={result.id} >
                  <div className='d-flex align-items-center gap-3 px-3 py-2 mb-0 hover-me'>
                        ...
                        className='img-fluid rounded-circle' style={{width: '40px' , height : '40px'}}/>
                        <div>
