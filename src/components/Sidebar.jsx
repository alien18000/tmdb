...
export default function Sidebar(setShowMenu) {
    ...
    console.log(genres)
  return (
    <div className='d-flex flex-column gap-2 px-lg-4 mt-lg-5 py-4 w-100'>
        <h1 className='text-secondary fs-6 mt-2 mb-1 mb-lg-2 px-2'>
            Discover
        </h1>
        {categories.map((category,index)=> (
            <NavLink to={`/${category.href}`} key={index} className={({isActive}) => isActive ? 'text-warning': 'text-white'} onClick={()=> setShowMenu(false)}>
...
<hr className='text-white'/>
                ...
                {genres.map((genre)=> (
                    <div key={genre.id} className='mb-0'>
                        <NavLink to={`/movies/genres/${genre.id}`} className={({isActive})=> isActive ? 
                        'text-warning' : 'text-white'} Click={()=> setShowMenu(false)}>
